...
if nargin<4,
   mn_bindesc=sprintf('(Bin %d+Bin %d)/2',binA,binB); 
end
if nargin<5,
   mn_ccode=[]; 
end
...
if isempty(use_subs),
    ...
    if isempty(mn_ccode)
        ...
        if length(GND.condesc)>=mn_ccode,
            exist_desc=1;
        else
            exist_desc=0;
        end
        if isempty(mn_ccdescr),
            if ~exist_desc,
               watchit(sprintf('There is no condition code descriptor for Condition Code %d.  ', ... 
                   mn_ccode));
               GND.condesc{mn_ccode}='Not Specified';
            end
        else
            if exist_desc,
                if ~strcmpi(mn_ccdescr,GND.condesc{mn_ccode}),
                    fprintf('Existing descriptor for Condition Code %d is "%s".\n', ...
                        mn_ccode,GND.condesc{mn_ccode});
                    ...
                    if strcmpi(resp,'y') || strcmpi(resp,'yes'),
                        GND.condesc{mn_ccode}=mn_ccdescr;
                    end
                end
            else
                GND.condesc{mn_ccode}=mn_ccdescr;
