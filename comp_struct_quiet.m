...
if nargin < 1; help comp_struct_quiet; error('I / O error'); end
...
%			clean display - add index if needed
			...
			[fss1 fss2 err] = comp_struct_quiet(getfield(s1(jj),char(fn1(pnt1i(ii)))), ...
...
%			different symbolic variables
	...
	if t
		%disp(sprintf('%s	%s		match',n1,n2)) DG Change ??
	else
...
%				tolerance agreement
				%disp(sprintf('%s 	%s		tolerance match',n1,n2)) DG Change ??
			end
		else
%			size difference
