...
p.addParamValue('force_chanlocs',[],@ischar);
...
if universal_yes(p.Results.force_chanlocs)
    unvsl_chanlocs=EEG.chanlocs;
else
    unvsl_chanlocs=[];
end
...
for filenum=1:n_infiles,
    ...
    if filenum>1,
        ...
        if ~isempty(unvsl_chanlocs)
            EEG.chanlocs=unvsl_chanlocs;
        end
