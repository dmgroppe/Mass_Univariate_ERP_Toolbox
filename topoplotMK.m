...
% 01-25-15 added a conditional that adjusts EMARKER2CHANS to reflect
% channels that are not visualized (e.g., because they have empty
% coordinates in the loc_file) DG
% 06-25: mark1chans and mark2chans flipped to be row vectors if they are
% column vectors DG
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    elseif length(Values) > length(tmpeloc)
       error('There are more Values than electrodes.'); % Groppe added this Jan 24, 2015 DG
    end
    ...
    % Groppe added the following conditional
    if length(Values)>length(plotchans)
       %some channels have been removed (e.g., if they did not have coordinates in the locfile)
       
       % Adjust EMARKER2CHAN indices to reflect missing channels
       tempVec=zeros(length(Values),1);
       tempVec(EMARKER2CHANS)=1;
       EMARKER2CHANS=find(tempVec(plotchans));
    end
    %
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    if strcmp(ELECTRODES,'on')   % plot electrodes as spots
        if isempty(EMARKER2CHANS)
            ...
            % DG added start
            if size(mark1chans,1)>1
                mark1chans=mark1chans';
            end
            if size(mark2chans,1)>1
